...
from elaspic import conf, elaspic_sequence, helper, standalone_pipeline, structure_tools
...
""".format(
        uniprot_id=uniprot_id, mutation=mutation, db_schema=conf.CONFIGS["db_schema"]
    )
    logger.debug(sql_query)
    df = pd.read_sql_query(sql_query, conf.CONFIGS["engine"])
    assert len(df) == 0, df
...
def run_database_pipeline(uniprot_id_mutation):
    ...
    sp = database_pipeline.DatabasePipeline(
        ...
        mutation,
        run_type="5",
        uniprot_domain_pair_ids=uniprot_domain_pair_ids,
    )
    sp.run()
