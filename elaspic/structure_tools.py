...
def get_structure_sequences(file_or_structure, seqres_sequence=False):
    ...
    if isinstance(file_or_structure, six.string_types):
        ...
        raise Exception(
            "Unexpected type {} for input ``file_or_structure`` {}!".format(
                file_or_structure, type(file_or_structure)
            )
        )
...
def get_interactions(model, chain_id, r_cutoff=6):
    ...
    for chain_id_2, chain_2 in model.child_dict.items():
        if chain_id == chain_id_2:
            continue
        interactions[chain_id_2] = get_interactions_between_chains(
            model, chain_id, chain_id_2, r_cutoff
        )
            ...
            for c in m:
                for r in c:
                    if r.is_disordered() or r.disordered:
                        logger.debug(
                            "Changing disordered_flag on residue {} from {} to 0".format(
                                r, r.disordered
                            )
                        )
                        r.disordered = 0
                    for a in r:
                        if a.is_disordered() or a.disordered_flag:
                            logger.debug(
                                "Changing disordered_flag on atom {} from {} to 0".format(
                                    a, a.disordered_flag
                                )
                            )
                            a.disordered_flag = 0
