...
from elaspic import (CACHE_DIR, conf, elaspic_database, elaspic_database_tables, elaspic_model,
                     elaspic_predictor, elaspic_sequence, errors, structure_tools)
    ...
    def _update_path_to_data(self, d_list):
        ...
        for d in d_list:
            if not d.path_to_data or any([len(x) > 255 for x in d.path_to_data.split('/')]):
                ...
                d.path_to_data = (elaspic_database.get_uniprot_base_path(d) +
    ...
    def run(self):
        ...
        if self.run_type in ['2', '4', '5', '6'] or 'model' in self.run_type:
            ...
            for d in self.uniprot_domains + self.uniprot_domain_pairs:
                self.get_model(d)
            logger.info('Finished processing all models for {} {}'
...
class _PrepareSequence:
    ...
    def __init__(self, d, db):
        ...
        if (d.uniprot_sequence and d.uniprot_sequence.provean and
                d.uniprot_sequence.provean.provean_supset_filename):
            # Provean supset has already been calculated
            self.provean_supset_file = op.join(conf.CONFIGS['archive_temp_dir'],
...
#                self.uniprot_domains = self.db.get_uniprot_domain(self.uniprot_id, True)
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if self.provean_supset_file is None:
            ...
            self.db.merge_provean(provean, self.sequence.provean_supset_file,
                                  conf.CONFIGS['copy_data'] and
                                  elaspic_database.get_uniprot_base_path(d))
...
class _PrepareModel:
    ...
    def __init__(self, d, db, new_model=False):
        ...
        if new_model:
            ...
            self.skip = True
        elif (isinstance(d, elaspic_database.UniprotDomainPair) and
            ...
            logger.info('Previous model had unfixable errors: "{}". Skipping...'
    ...
    def _get_modeller_results_file(self, d):
        ...
        if isinstance(d, elaspic_database.UniprotDomain):
            ...
            modeller_results['model_file'] = op.join(
                conf.CONFIGS['archive_temp_dir'], d.path_to_data, d.template.model.model_filename)
        ...
        modeller_results_file = op.join(conf.CONFIGS['model_dir'],
    ...
    def __enter__(self):
        ...
        if isinstance(d, elaspic_database.UniprotDomain):
            ...
            protein_ids = [d.uniprot_domain_1.uniprot_id, d.uniprot_domain_2.uniprot_id]
            if (d.template.model and d.template.model.model_domain_def_1 and
                    d.template.model.model_domain_def_2):
                protein_domain_defs = [
                    d.template.model.model_domain_def_1, d.template.model.model_domain_def_2
                ]
            else:
                protein_domain_defs = [
                    d.uniprot_domain_1.template.domain_def, d.uniprot_domain_2.template.domain_def
                ]
            protein_sequences = [
            ...
            pdb_chains = [d.template.domain_1.pdb_chain, d.template.domain_2.pdb_chain]
            pdb_domain_defs = [
        ...
        self.structure_file, self.structure_seqrecords = (self._write_domain_structure_file(
            pdb_id, pdb_chains, pdb_domain_defs))
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        d = self.d
        if (exc_type is not None and exc_type in self.bad_errors and
            ...
            if isinstance(d, elaspic_database.UniprotDomain):
                ...
                bad_domains = self.db.get_rows_by_ids(elaspic_database_tables.Domain,
                ...
                logger.error("Making a homology model failed!!!\n"
                             "Adding error '{0}' to the domain with cath_id {1}...".format(
                                 bad_domain.domain_errors, d.template.cath_id))
                ...
                bad_domains = self.db.get_rows_by_ids(elaspic_database_tables.DomainContact, [
            ...
            d.template.model.model_errors = (self.__add_new_error(d.template.model.model_errors,
                                                                  exc_value))
        ...
        if isinstance(d, elaspic_database.UniprotDomain):
            ...
            d.template.model.model_domain_def = (self._truncate_domain_defs(
                d.template.domain_def, self.model.modeller_results['domain_def_offsets'][0]))
        ...
        # Domain pairs
            ...
            d.template.model.interacting_aa_1 = (','.join(
                [str(uniprot_num) for uniprot_num in chain_1_interacting_uninum]))
            d.template.model.interacting_aa_2 = (','.join(
                [str(uniprot_num) for uniprot_num in chain_2_interacting_uninum]))
    ...
    def _truncate_domain_defs(self, domain_def, domain_def_offset):
        ...
        if n_gaps_end is None:
            n_gaps_end = 0
        domain_def_new = (str(int(domain_def.split(':')[0]) + n_gaps_start) + ':' +
...
class _PrepareMutation:
    ...
    def __init__(self, d, mutation, uniprot_id, sequence, model, db):
        ...
        if not mutation:
            ...
            self.skip = True
              ...
              (isinstance(d, elaspic_database_tables.UniprotDomainPair) and
        ...
        if (precalculated_mutation and precalculated_mutation.provean_score and
    ...
    def run(self):
        d = self.d
        _domain_tables = (elaspic_database_tables.UniprotDomain,
        ...
        self.mut.model_filename_wt = ('{}_{}/{}'.format(self.uniprot_id, self.mutation,
                                                        op.basename(results['model_file_wt'])))
        self.mut.model_filename_mut = ('{}_{}/{}'.format(self.uniprot_id, self.mutation,
                                                         op.basename(results['model_file_mut'])))
        # Copy models to a 'archive_temp_dir`, so that `database.merge_mutation` knows where to
        # find them.
        archive_model_file_wt = (op.join(conf.CONFIGS['archive_temp_dir'], d.path_to_data,
                                         self.mut.model_filename_wt))
        archive_model_file_mut = (op.join(conf.CONFIGS['archive_temp_dir'], d.path_to_data,
    ...
    def get_mutation_data(self):
        ...
        if isinstance(d, elaspic_database.UniprotDomain):
            ...
            if (uniprot_id_1 == d.uniprot_domain_1.uniprot_id and
                ...
                error_message = ('Mutated residue {} not involved in the interaction!'
                                 .format(mutation[:-1]))
    ...
    def get_mutation_features(self, d, mut, row_idx=0):
        ...
        feature_dict = {
            key: value
            for (key, value) in mut.__dict__.items() if not key.startswith('_')
        }
        ...
        if hasattr(d, 'uniprot_domain_id'):
            ...
            feature_dict.update({
                ...
                'clan_name_1':
                d.uniprot_domain_1.pfam_clan,
                'clan_name_2':
                d.uniprot_domain_2.pfam_clan,
                # Feature columns
                'alignment_identity':
                np.sqrt(d.template.identical_1 * d.template.identical_2),
                'alignment_coverage':
                np.sqrt(d.template.coverage_1 * d.template.coverage_2),
                'alignment_score':
                np.sqrt(d.template.score_1 * d.template.score_2),
                #
                ...
                d.template.model.interface_area_total,
            })
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        d = self.d
        mutation = self.mutation
        if (exc_type is not None and exc_type in (errors.MutationOutsideDomainError,
        ...
        for attr in dir(self.mut):
            ...
            if isinstance(attr_field, six.binary_type):
                logger.debug('Changing attribute {} from {} to {}...'
