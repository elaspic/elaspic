...
class DatabasePipeline(Pipeline):
    ...
    def run(self):
        ...
        # Find provean
        if (
            self.run_type in ["1", "5", "6"] or "sequence" in self.run_type
        ) and self.uniprot_domains:
            logger.info("\n\n\n" + "*" * 110)
...
class _PrepareSequence:
    def __init__(self, d, db):
        ...
        if (
            d.uniprot_sequence
            and d.uniprot_sequence.provean
...
class _PrepareModel:
    ...
    def __init__(self, d, db, new_model=False):
        ...
        if new_model:
            ...
            d.template.model
            and d.template.model.model_errors
            and (
                ("Giving up" in d.template.model.model_errors)
                or (d.template.model.model_errors.count(";") >= 2)
            )
        ):
            logger.info(
                'Previous model had unfixable errors: "{}". Skipping...'.format(
    ...
    def _get_modeller_results_file(self, d):
        ...
        modeller_results_file = op.join(
            conf.CONFIGS["model_dir"], "{}_modeller_results.json".format(unique_id)
        )
    ...
    def __enter__(self):
        ...
        if isinstance(d, elaspic_database.UniprotDomain):
            ...
            if (
                d.template.model
                and d.template.model.model_domain_def_1
                and d.template.model.model_domain_def_2
            ):
                protein_domain_defs = [
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if (
            ...
            and exc_type in self.bad_errors
            and self.modeller_results_file is not None
        ):
            ...
            d.template.model.model_errors = self.__add_new_error(
                d.template.model.model_errors, exc_value
            )
        ...
        if isinstance(d, elaspic_database.UniprotDomain):
            ...
            d.template.model.alignment_filename = op.basename(
                self.model.modeller_results["alignment_files"][0]
            )
        ...
        # Domain pairs
            ...
            d.template.model.alignment_filename_1 = op.basename(
                self.model.modeller_results["alignment_files"][0]
            )
            d.template.model.alignment_filename_2 = op.basename(
                self.model.modeller_results["alignment_files"][1]
            )
...
class _PrepareMutation:
    ...
    def run(self):
        ...
        self.mut.model_filename_wt = "{}_{}/{}".format(
            self.uniprot_id, self.mutation, op.basename(results["model_file_wt"])
        )
        self.mut.model_filename_mut = "{}_{}/{}".format(
            self.uniprot_id, self.mutation, op.basename(results["model_file_mut"])
        )
        # Copy models to a 'archive_temp_dir`, so that `database.merge_mutation` knows where to
        # find them.
        archive_model_file_wt = op.join(
            conf.CONFIGS["archive_temp_dir"], d.path_to_data, self.mut.model_filename_wt
        )
        archive_model_file_mut = op.join(
            conf.CONFIGS["archive_temp_dir"],
            d.path_to_data,
    ...
    def get_mutation_data(self):
        ...
        if isinstance(d, elaspic_database.UniprotDomain):
            ...
            if (
                ...
                logger.error(
                    "Uniprot ID 2: {}\tInteracting AA 2: {}".format(
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        for attr in dir(self.mut):
            ...
            if isinstance(attr_field, six.binary_type):
                logger.debug(
                    "Changing attribute {} from {} to {}...".format(
                        attr, attr_type, type(attr_field.decode())
                    )
                )
