...
class PipelineTemplate(object):
    ...
    def __copy_blast_database(self):
        if (os.path.isdir(conf.configs['global_temp_path'] + 'blast/db/') and
            ...
            os.path.isfile(conf.configs['global_temp_path'] + 'blast/db/pdbaa.pal')):
                ...
                if result.strip() == 'False':
                    ...
                    logger.info('A local copy of the blast database already exists. Good!')
                else:
                    raise Exception(
        ...
        while return_code != 0 and n_tries < 10:
            logger.info("Couldn't copy the blast database. Retrying...")
            logger.debug(result)
    ...
    #==============================================================================
    # Helper methods for functions that will inherit this.
    #==============================================================================
    ...
    def _prepare_provean_temp_folder(self):
        """
        ...
        if conf.configs['provean_temp_path'] and not (('beagle' in hostname) or ('grendel' in hostname)):
            ...
            provean_temp_path = op.join(conf.configs['provean_temp_path'], self.unique) + '/'
        else:
            provean_temp_path = op.join(self.unique_temp_folder, 'provean_temp') + '/'
        subprocess.check_call('mkdir -p ' + provean_temp_path, shell=True)
        return provean_temp_path


    def _clear_provean_temp_files(self):
        logger.debug("Clearning provean temporary files from '{}'".format(self.provean_temp_path))
        try:
...
class Pipeline(PipelineTemplate):
    ...
    def __call__(self, uniprot_id, mutations, run_type=1, number_of_tries=[], uniprot_domain_pair_ids=[]):
        ...
        logger.info('unique_temp_folder: {}'.format(self.unique_temp_folder))
        ...
        self.provean_temp_path = self._prepare_provean_temp_folder()
        atexit.register(self._clear_provean_temp_files)
        ...
        self.db = sql_db.MyDatabase(conf.configs)
        ...
        if run_type in [1, 5] and self.uniprot_domains:
            ...
            if self._compute_provean():
                if run_type == 1:
                    logger.info('Finished run_type {}'.format(run_type))
                    return
        ...
        if run_type in [2, 4, 5]:
            self.get_model = domain_model.GetModel(
                self.unique_temp_folder, self.db, conf.configs)
            logger.info('\n\n\n' + '*' * 110)
        ...
        if run_type in [3, 4, 5] and self.mutations:
            self.get_mutation = domain_mutation.GetMutation(
                self.unique_temp_folder, self.db, self.provean_temp_path, conf.configs)
            logger.info('\n\n\n' + '*' * 110)
                ...
                if os.path.isfile(path_to_provean_supset):
                    ...
                    result, error_message, return_code = \
                        domain_alignment.check_provean_supporting_set(
                            ...
                            logger, d.uniprot_id, path_to_provean_supset,
