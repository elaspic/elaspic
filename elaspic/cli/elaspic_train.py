...
def _train_predictor(prefix):
    ...
    assert prefix in elaspic.Predictor.types
    # Load data
    data = pd.read_csv(
        op.join(elaspic.DATA_DIR, '{}_training_set.tsv.gz'.format(prefix)),
        sep='\t', low_memory=False)
    with open(op.join(elaspic.DATA_DIR, '{}_features.json'.format(prefix)), 'rt') as fh:
        features = json.load(fh)
    with open(op.join(elaspic.DATA_DIR, '{}_options.json'.format(prefix)), 'rt') as fh:
        options = json.load(fh)
    # Train
    core_predictor = elaspic.Predictor().get_final_predictor(data, features, options)
    # Save
    with open(op.join(elaspic.CACHE_DIR, '{}_clf.pickle'.format(prefix)), 'wb') as fh:
