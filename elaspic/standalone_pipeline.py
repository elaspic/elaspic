...
class StandalonePipeline(Pipeline):
    ...
    def __init__(
        ...
        if len(self.sp.chain_ids) != len(self.seqrecords):
            logger.warning(
                "The number of chain ids ({}) does not match the number of sequences ({})!".format(
                    len(self.sp.chain_ids), len(self.seqrecords)
                )
            )
    ...
    def _parse_mutations(self, mutations, mutation_format="1"):
        ...
        for mutation_in in mutations:
            ...
            if mutation_format in ["1"]:
                ...
                resnum_suffix = "".join(
                    n for n in mutation_residue[1:-1] if not n.isdigit()
                ).upper()
                mutation_id = (" ", resnum, resnum_suffix or " ")
                chain_aa_residues = structure_tools.get_aa_residues(
                ...
                # Validation
                mutation_expected_aa = str(self.seqrecords[mutation_idx].seq)[
                    int(mutation[1:-1]) - 1
                ]
    ...
    def run_all_mutations(self):
        ...
        for (mutation_idx, mutation), mutation_in in self.mutations.items():
            ...
            if op.isfile(mutation_results_file):
                logger.debug(
                    "Results file for mutation {} already exists: {}".format(
                        mutation_in, mutation_results_file
                    )
                )
                continue
            try:
...
class PrepareSequence:
    ...
    def __enter__(self):
        sequence_file = op.join(
            conf.CONFIGS["sequence_dir"], helper.slugify(self.seqrecord.id + ".fasta")
        )
