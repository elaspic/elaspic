...
class Predictor:
    ...
    # TODO: We should be able to register more Predictor types dynamically.
    types = ['core', 'interface']
    ...
    @classmethod
    def format_mutation_features(cls, df):
        """.
        ...
        -------
        DataFrame
            Contains the same data as `feature_df`, but with columns containing
            comma-separated lists of features converted to columns containing
            a single feature each.
    ...
    @classmethod
    def convert_features_to_differences(cls, df, keep_mut=False):
        """Convert `_wt` and `_mut` columns into `_wt` and `_change` columns.

        Create a new set of features (ending in `_change`) that describe
        the difference between values of the wildtype (features ending in `_wt`)
        and mutant (features ending in `_mut`) features.
    ...
    @classmethod
    def get_core_mutations(cls, df, engine, schema_name='elaspic'):
        values = (
    ...
    """.format(schema_name=schema_name, values=', '.join(values))
        ...
        results_df = cls.format_mutation_features(results_df)
        # keep `_wt` and `_mut`, remove later:
        results_df = cls.convert_features_to_differences(results_df, True)

        return results_df

    @classmethod
    def get_interface_mutations(cls, df, engine, schema_name='elaspic'):
        values = (
            ("('" + df['uniprot_id'] + "', '" + df['uniprot_mutation'] + "')")
        )
        sql_query = """\
