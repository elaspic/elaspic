...
def format_mutation_features(df):
    ...
    for column_index, column_name in enumerate(names_phys_chem):
        df[column_name + "_wt"] = df["physchem_wt"].apply(
            lambda x: float(x.split(",")[column_index]) if pd.notnull(x) else np.nan
        )
        df[column_name + "_self_wt"] = df["physchem_wt_ownchain"].apply(
            lambda x: float(x.split(",")[column_index]) if pd.notnull(x) else np.nan
        )
        df[column_name + "_mut"] = df["physchem_mut"].apply(
            lambda x: float(x.split(",")[column_index]) if pd.notnull(x) else np.nan
        )
        df[column_name + "_self_mut"] = df["physchem_mut_ownchain"].apply(
            lambda x: float(x.split(",")[column_index]) if pd.notnull(x) else np.nan
        )
    del df["physchem_wt"]
    ...
    if (foldx_interface_column_name + "_wt") in df.columns and df[
    ...
    df_core = _split_foldx_features(
        df_core, foldx_core_column_name + "_wt", call_foldx.names_stability_wt
    )
    df_core = _split_foldx_features(
        df_core, foldx_core_column_name + "_mut", call_foldx.names_stability_mut
    )
...
def convert_features_to_differences(df, keep_mut=False):
    ...
    for column_name, column in df.iteritems():
        if (
            "_mut" in column_name
            and column_name.replace("_mut", "_wt") in df.columns
...
def get_final_predictor(data, features, options):
    ...
    if len(data) != len(data_usable):
        warnings.warn(
            "Warning, {} rows in the provided data contained null!".format(
                len(data) - len(data_usable)
            )
        )
    data = data_usable
