...
# File paths
file_path = '/home/alexey/kimstg/database_data/pdb/ftp/data/structures/divided/mmCIF'
output_folder = '/home/alexey/working/databases/mysql-query-outputs/RealPDBPfam27Table'
temp_filename = output_folder + '/' + 'tmpfile.cif'
...
logger = logging.getLogger('parse_mmcifs')
logger.setLevel(logging.ERROR)

handler = logging.FileHandler(output_folder + '/' + "errors.log", "w")
...
# Inputs
inputs = np.recfromtxt(arguments.input_file, delimiter='\t')
    ...
    for domain_def in domain_defs:
        ...
        for residue in chain.get_residues():    
            ...
            if residue.id[1] == domain_def[1]+1:
                reached_end = True
                if not passed_start:
                    logger.debug(id_for_logger + ' reached the last AA of a domain without going through the first!')
                    errors_occured = True
...
#                print residue.id
                if residue.resname in aa_translations.keys():
                    ...
                    logger.debug(id_for_logger + ' an unusual AA was found in the domain!')
                    errors_occured = True
        ...
        if not reached_end:
            error_list.append((pdb_id, pdb_chain, domain_def))
            logger.debug(id_for_logger + ' reached the end of chain before reaching the last AA of domain!')
            errors_occured = True
    ...
    if len(domainAA_string) - domainAA_string.count('/') != sum([y+1-x for (x,y) in domain_defs]):
        logger.debug(id_for_logger + ' length of AA sequence does not match the length of domain!')
        errors_occured = True
