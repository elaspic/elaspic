...
#            host_name, __ = childProcess.communicate()
    ...
    def __call__(self, uniprot_id, mutation):
        ...
        self.get_template = domain_template.GetTemplate(self.tmpPath, self.unique, self.pdbPath, self.db, self.log)
            
        self.get_model = domain_model.GetModel(self.tmpPath, self.unique, self.pdbPath, self.db, self.log, 
            self.modeller_runs, self.buildModel_runs, self.PWD)
            
        self.get_mutation = domain_mutation.GetMutation(self.tmpPath, self.unique, self.pdbPath, self.db, self.log, 
    ...
    def _compute_provean(self):
        ...
        for p in self.protein_definitions:
            ...
            if not p.t:
                self.log.info('skipping template')
                continue                
            elif not isinstance(p.t, sql_db.UniprotDomainTemplate) \
            or not p.t.alignment_filename \
            or p.t.template_errors \
            or p.t.provean_supset_filename:
                self.log.info('skipping template')
                continue
            ...
            try:
                ...
                self.log.error(e.message)
                template.template_errors = e.message
            ...
            for mutation in self.mutations.split(','):
                precalculated_mutation = self.db.get_uniprot_mutation(p.m, self.uniprot_id, mutation)
                self.log.info(precalculated_mutation)
                if precalculated_mutation and precalculated_mutation.provean_score:
                ...
                if type(p.d) == sql_db.UniprotDomain:
                    ...
                    if not p.t.provean_supset_filename:
                        self.log.error('No provean supset found. Skipping...')
                        continue
                    empty_mutation = sql_db.UniprotDomainMutation()
                    empty_mutation.uniprot_domain_id = p.d.uniprot_domain_id
                    path_to_provean_supset = self.tmpPath + p.d.path_to_data + p.t.provean_supset_filename
                    
                elif type(p.d) == sql_db.UniprotDomainPair:
                    if not p.t.domain_def_1 or not p.t.domain_def_2:
                        self.log.error('At least one of the partners is missing domain definitions')
                        self.log.error(precalculated_mutation)
                        continue
                    if self.uniprot_id == p.d.uniprot_domain_1.uniprot_id:
                        d_1 = p.d.uniprot_domain_1
                    elif self.uniprot_id == p.d.uniprot_domain_2.uniprot_id:
                        d_1 = p.d.uniprot_domain_2
                    else:
                        self.log.error('Query uniprot id does not match either of the domain uniprot ids')
                    if not d_1.template[0].provean_supset_filename:
                        self.log.error('No provean supset found. Skipping...')
                        continue
                ...
                provean_mutation, provean_score = self.get_mutation.get_provean_score(
                    empty_mutation.uniprot_domain_pair_id, d_1, domain_sequences[0], path_to_provean_supset)
                empty_mutation.uniprot_id = self.uniprot_id
                empty_mutation.mutation = mutation
                
                try:
                    uniprot_mutation = self.get_mutation(p.d, p.t, p.m, self.uniprot_id, mutation)
...
#                    continue
        ...
        self.log.info('Finished adding provean info to all templates for ' + self.uniprot_id + ' ' + self.mutations + '\n')
    ...
    def _compute_models(self):            
        ...
        for p in self.protein_definitions:
            ...
            if (type(p.t) == sql_db.UniprotDomainTemplate and not p.t.domain_def) \
            or (type(p.t) == sql_db.UniprotDomainPairTemplate and (not p.t.domain_def_1 or not p.t.domain_def_2)):
                ...
                errors.ChainsNotInteracting) as e:
                    ...
                    uniprot_model = empty_model
                    continue
#                    uniprot_model.model_errors = e.message
