...
#            self.log.debug('child id after:' + child.id)
    ...
    def _run_msms(self, filename):
        ...
        self.log.debug('entered %s' % self.working_path)
        ...
        with open('standard.data', 'r') as fh:
...
#                            atom_to_atom_id[atom.serial_number] = atom.id
        ...
        system_command = ('./pdb_to_xyzrn %s > %s' % (base_filename + '.pdb', base_filename + '.xyzrn',))
        ...
        system_command = (
            './msms.x86_64Linux2.2.6.1 -probe_radius 1.4 -surface ases -if %s -af %s' % 
            (base_filename + '.xyzrn', base_filename + '.area',))
        ...
        with open(base_filename + '.area', 'r') as fh:
...
#        del seasa_df['res_num_msms']
    ...
    def _run_naccess_atom(self, filename):
        ...
        rc = childProcess.returncode
        self.log.debug('naccess result:')
        self.log.debug(result)
        self.log.debug('naccess error:')
        self.log.debug(e)
        self.log.debug('naccess rc:')
        self.log.debug(rc)
        # collect results
        sasa_scores = {}
...
###############################################################################
    ...
    def __run_pops_area(self, full_filename):
        ...
        self.log.debug('pops result: %s' % result)
        self.log.debug('pops error: %s' % e)
        self.log.debug('pops rc: %s' % rc)
        return output[-1], rc, e
...
# We now calculate dssp and interface amino acids for the entire model
    def __check_structure(self, pdbCode, chainID, mutation):
        ...
        """
        structure = pdb_template.get_pdb(pdbCode, self.pdbPath)
        model = structure[0]
